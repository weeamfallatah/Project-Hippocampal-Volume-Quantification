...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90), txt,(255, 255, 255), font=main_font)
...
def get_series_for_inference(path):
    ...
    for dcm in dicoms:
        ...
        pid = getattr(dcm, "PatientID", "N/A")
        #sid = getattr(dcm, "SeriesInstanceUID", "N/A")
        print(f"File:  PatientID: {pid}")
        
    series_dict = {}
    for dcm in dicoms:
        sid = dcm.SeriesInstanceUID
        series_dict.setdefault(sid, []).append(dcm)

    # فلترة: نبحث عن أول سلسلة تحتوي على SeriesDescription = HippoCrop
    for series in series_dict.values():
        desc = getattr(series[0], "SeriesDescription", "")
        if "HippoCrop" in desc:
            print(f"Selected series: {desc}, UID: {series[0].SeriesInstanceUID}")
            return series

    print("Error: Could not find a series with 'HippoCrop' in SeriesDescription.")
    return []
...
if __name__ == "__main__":
    ...
    os_command(f"sudo storescu localhost 4242 -v -aec HIPPOAI +r +sd {report_save_path}")
